...
class Dwelling:
    def __init__(self, rooms, square_feet, floors):
...
class SingleFamilyHome(Dwelling):
    def __init__(self, rooms, square_feet, floors, garage_type, yard_size):
        Dwelling.__init__(self, rooms, square_feet, floors)
...
#            6 rooms, 1200 square feet, 1 floor, single car garage, .25 acres
    House = SingleFamilyHome(6, 1200, 1, "single car", ".25 acres")
# 8) Display the data using the accessor methods
    print(f"rooms: {House.get_rooms()}, square feet: {House.get_feet()}, floors: {House.get_floors()}, garage: {House.get_garage()}, yard: {House.get_yard()}")
